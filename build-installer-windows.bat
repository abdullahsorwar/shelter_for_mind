...
copy /Y "target\shelter_of_mind-1.0.jar" "target\libs\"
if errorlevel 1 goto error

echo.
echo Step 5: Creating Windows MSI installer with full Java runtime...
call jpackage --input target\libs --name "Shelter for Mind" --main-jar shelter_of_mind-1.0.jar --main-class com.the_pathfinders.Launcher --type msi --app-version 1.0 --vendor "the_pathfinders" --description "Mental Health and Wellness Platform" --icon app-icon.ico --win-dir-chooser --win-menu --win-shortcut --java-options "-Dfile.encoding=UTF-8" --runtime-image "%JAVA_HOME%"
if errorlevel 1 goto error
