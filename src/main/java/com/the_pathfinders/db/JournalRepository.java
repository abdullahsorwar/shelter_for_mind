...
public class JournalRepository {
    ...
    /**
     * Update an existing journal's text and style properties.
     * @return true if update affected a row
     */
    public boolean updateJournal(String journalId, String journalText, String fontFamily, Integer fontSize) throws SQLException {
        String sql = "UPDATE public_journals SET journal_text = ?, font_family = ?, font_size = ? WHERE journal_id = ?";
        try (Connection conn = DB.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, journalText);
            ps.setString(2, fontFamily);
            ps.setInt(3, fontSize);
            ps.setString(4, journalId);
