...
public class LoginSignupController implements Initializable {
    ...
    // inject this from App
    private SoulRepository repo;

    public void setRepository(SoulRepository repo) {
        this.repo = repo;
    }
    ...
    @FXML
    private TextField tfLoginId;
    @FXML
    private PasswordField pfLoginKey;
    @FXML
    private Label errLoginId, errLoginKey, lblLoginStatus;
    @FXML
    private Button btnLoginSubmit;
    @FXML
    private TextField tfLoginKeyVisible;
    @FXML
    private CheckBox cbShowLoginKey;

    // Sign up form
    @FXML
    private VBox signupForm;
    @FXML
    private TextField tfSoulName, tfSoulId, tfMobile;
    @FXML
    private PasswordField pfSoulKey;
    @FXML
    private DatePicker dpDob;
    @FXML
    private ComboBox<String> cbCountryCode;
    @FXML
    private Label errSoulName, errDob, errSoulId, errSoulKey, errMobile, lblSubmitStatus;
    @FXML
    private Button btnSubmit;
    @FXML
    private TextField tfSoulKeyVisible;
    @FXML
    private CheckBox cbShowSignupKey;
    ...
    // Tab state
    private boolean isLoginSelected = true; // app opens on Login
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        // Tab button actions
        signUpBtn.setOnAction(e -> switchMenu(false)); // show Sign Up
        loginBtn.setOnAction(e -> switchMenu(true)); // show Log in
        ...
        Platform.runLater(() -> {
            ...
            resnap.run();
            Platform.runLater(resnap); // one more tick after CSS
        });
        // bind visible-plain textfields to their password fields
    ...
    private void playIntroAnimation() {
        ...
        logoImage.setScaleX(1.5);
        logoImage.setScaleY(1.5);
        titleLabel.setOpacity(0);
        titleLabel.setScaleX(1.5);
        titleLabel.setScaleY(1.5);
        subtitleLabel.setOpacity(0);
        subtitleLabel.setScaleX(1.5);
        subtitleLabel.setScaleY(1.5);
        ...
        scaleLogo.setFromX(1.5);
        scaleLogo.setFromY(1.5);
        ...
        scaleTitle.setFromX(1.5);
        scaleTitle.setFromY(1.5);
        ...
        scaleSubtitle.setFromX(1.5);
        scaleSubtitle.setFromY(1.5);
        scaleSubtitle.setToX(1.0);
        scaleSubtitle.setToY(1.0);

        new ParallelTransition(fadeLogo, scaleLogo, fadeTitle, scaleTitle, fadeSubtitle, scaleSubtitle).play();
    }

    // Cross-fade only so layout never shifts
    ...
    /** Coordinates computed in menuBorder local space (works across resizes). */
    ...
    private void snapBorderTo(Button target) {
        applyToElements(computeGeometryFor(target));
    }
    ...
    private void switchMenu(boolean toLogin) {
        if (isLoginSelected == toLogin)
            return; // no-op if same tab
        wasLoginSelected = isLoginSelected;
        isLoginSelected = toLogin;

        // reset fields/errors on the target tab so it opens clean
        if (toLogin)
            resetLoginForm();
        else
            resetSignupForm();
    ...
    private void applyTabStyles(boolean loginSelectedNow) {
        if (loginSelectedNow) {
            loginBtn.setStyle(
                    "-fx-background-color: transparent; -fx-font-size: 15px; -fx-font-weight: bold; -fx-text-fill: #1f1f1f;");
            signUpBtn.setStyle(
                    "-fx-background-color: transparent; -fx-font-size: 15px; -fx-font-weight: normal; -fx-text-fill: #1f1f1f;");
        } else {
            signUpBtn.setStyle(
                    "-fx-background-color: transparent; -fx-font-size: 15px; -fx-font-weight: bold; -fx-text-fill: #1f1f1f;");
            loginBtn.setStyle(
    ...
    private void setRightContentForCurrentTab() {
        ...
        signupForm.setManaged(showSignup);
        loginForm.setVisible(!showSignup);
        loginForm.setManaged(!showSignup);
    }
    ...
    private void onSubmitSignup() {
        ...
        if (soulId.isEmpty() || !soulId.matches("^[A-Za-z0-9_]+$")) {
            errSoulId.setText("ERROR! Soul_ID can contain A-Z, a-z, 0-9 and _ only");
            return;
        }
        ...
        try {
            ...
            if (repo.idExists(soulId)) {
                lblSubmitStatus.setText("ERROR! Duplicate id found! Enter a new one");
                lblSubmitStatus.setStyle("-fx-text-fill: #e53935; -fx-font-size: 12px;");
                return;
            }
            ...
            lblSubmitStatus.setStyle("-fx-text-fill: #00ff00ff; -fx-font-size: 12px;");
        } catch (DuplicateIdException dup) {
            lblSubmitStatus.setText("ERROR! Duplicate id found! Enter a new one");
            lblSubmitStatus.setStyle("-fx-text-fill: #e53935; -fx-font-size: 12px;");
        } catch (Exception ex) {
            lblSubmitStatus.setText("ERROR! Server unavailable. Try again.");
            lblSubmitStatus.setStyle("-fx-text-fill: #e53935; -fx-font-size: 12px;");
            ex.printStackTrace();
        }
    }
    ...
    private void onSubmitLogin() {
        ...
        if (id.isEmpty() || !id.matches("^[A-Za-z0-9_]+$")) {
            errLoginId.setText("ERROR! Soul_ID can contain A-Z, a-z, 0-9 and _ only");
            return;
        }
        ...
        try {
            ...
            if (!ok) {
                lblLoginStatus.setText("ERROR! Invalid id or key");
                lblLoginStatus.setStyle("-fx-text-fill: #e53935; -fx-font-size: 12px;");
                return;
            }
            lblLoginStatus.setText("Success! You are logged in!");
            lblSubmitStatus.setStyle("-fx-text-fill: #00ff00ff; -fx-font-size: 12px;");
        } catch (Exception ex) {
            lblLoginStatus.setText("ERROR! Server unavailable. Try again.");
            lblLoginStatus.setStyle("-fx-text-fill: #e53935; -fx-font-size: 12px;");
            ex.printStackTrace();
        }
    }
    ...
    private void resetLoginForm() {
        ...
        // hide plain-text view if it was on
        try {
            toggleShowLoginKey(false);
